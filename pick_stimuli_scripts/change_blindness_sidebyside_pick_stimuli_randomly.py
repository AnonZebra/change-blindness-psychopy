...
# path to CSV file describing all images already used in the 'flicker' version of the
# experiment
FLICKER_SPEC_PATH = "PATH/TO/change_blindness_flicker_stimuli_data.csv"
...
# get a set containing all numbers of images already used in the 'flicker' version
# of the experiment
flicker_df = pd.read_csv(FLICKER_SPEC_PATH)
flick_fnames = flicker_df.target_present
flick_nums = [int(re.match(r'^[\d]+', x)[0]) for x in flick_fnames]
flick_nums = set(flick_nums)
# exclude numbers that are already used for flicker version
nonflick_nos = all_nos - flick_nums



# get a set of randomly picked image file numbers, that will be used for selecting 
# images with the change on the left side
left_nos = set(random.sample(nonflick_nos, 12)) 
...
right_nos = set(random.sample(nonflick_nos - left_nos, 12))  
...
# now that we're certain that there are 12 left/right images each, we can merge the lists
...
size_loc_df = pd.read_excel("CBDatabase_Size_Eccentricity_RT.xlsx", dtype=str)
...
img_path_df.to_csv('change_blindness_sidebyside_stimuli_data.csv', index=False)
